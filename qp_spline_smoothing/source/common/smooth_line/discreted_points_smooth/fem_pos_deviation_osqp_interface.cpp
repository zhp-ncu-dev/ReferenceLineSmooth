...
namespace planning
{
    bool FemPosDeviationOsqpInterface::Solve()
    {
        ...
        //uint64_t count = std::numeric_limits<int>::max();  // maxnum is 2147483647
        ...
        // Set primal warm start 以参考点为 warm_start 点
            ...
            std::vector<c_int>* P_indptr)
    {
        ...
        // Only upper triangle is filled 【只填充上三角】
    ...
    // - 2*x(i)_ref*x(i) - 2*x(i)_ref*x(i)
    // - 2*y(i)_ref*y(i) - 2*y(i)_ref*y(i)
    ...
    void FemPosDeviationOsqpInterface::CalculateAffineConstraint(
            ...
            std::vector<c_float>* upper_bounds)
    {
        // x_lower(i) <= x(i) <= x_upper(i)
        // y_lower(j) <= y(j) <= y_upper(j)
        int ind_A = 0;
