...
#include "common/smooth_line/smooth_spline/spline_2d_constraint.h"

namespace planning
{
    Spline2dConstraint::Spline2dConstraint(const std::vector<double>& t_knots, const uint32_t order)
    {
        t_knots_ = t_knots;
        spline_order_ = order;
            ...
            const std::vector<double>& lateral_bound)
    {
        ...
        for (uint32_t i = 0; i < t_coord.size(); ++i)
        {
            ...
            const double d_longitudinal =
                    SignDistance(ref_point[i], angle[i]);
            ...
            std::vector<double> longi_coef = AffineCoef(angle[i] + M_PI / 2.0, rel_t);
            std::vector<double> longitudinal_coef =
                    AffineCoef(angle[i], rel_t);
    ...
    double Spline2dConstraint::SignDistance(const math::Vec2d& xy_point,
                        const double& angle) const
    {
       return math::InnerProd(xy_point.x(), xy_point.y(), std::sin(angle),
                  std::cos(angle));
    }
    ...
    uint32_t Spline2dConstraint::FindIndex(const double t) const
    {
        ...
        return std::min(static_cast<uint32_t>(t_knots_.size() - 1),
                 static_cast<uint32_t>(upper_bound - t_knots_.begin())) - 1;
    }
    ...
    std::vector<double> Spline2dConstraint::AffineDerivativeCoef(
        ...
        const uint32_t num_params = spline_order_ + 1;
        std::vector<double> result(num_params * 2, 0.0);
        double x_coef = std::cos(angle);
        double y_coef = -std::sin(angle);
    ...
    std::vector<double> Spline2dConstraint::AffineDerivativeCoef(const double t) const
    {
        const uint32_t num_params = spline_order_ + 1;
        std::vector<double> result(num_params, 0.0);
        std::vector<double> power_t = PolyCoef(t);
        for (uint32_t i = 1; i < num_params; ++i)
        {
    ...
    std::vector<double> Spline2dConstraint::AffineSecondDerivativeCoef(const double t) const
    {
        const uint32_t num_params = spline_order_ + 1;
        std::vector<double> result(num_params, 0.0);
        std::vector<double> power_t = PolyCoef(t);
    ...
    bool Spline2dConstraint::AddPointSecondDerivativeSmoothConstraint(const double& t,
                                                  const double& xds,
                                                  const double& yds,
                                                  ...
                                                  const double& yseconds)
    {
        ...
        std::vector<double> derivative_coef = AffineDerivativeCoef(rel_t);
        std::vector<double> second_derivative_coef = AffineSecondDerivativeCoef(rel_t);
        for (uint32_t i = 0; i < derivative_coef.size(); ++i) {
            affine_equality(0, i + index_offset) = derivative_coef[i];
            affine_equality(1, i + index_offset + num_params) = derivative_coef[i];
//            affine_equality(2, i + index_offset) = second_derivative_coef[i];
//            affine_equality(3, i + index_offset + num_params) = second_derivative_coef[i];
        }
        affine_boundary(0,0) = xds;
        affine_boundary(1,0) = yds;
//        affine_boundary(2,0) = xseconds;
//        affine_boundary(3,0) = yseconds;
        if (!AddEqualityConstraint(affine_equality, affine_boundary)) {
            std::cout<<"添加不同时刻的一阶,二阶连续失败"<<std::endl;
            return false;
        }
        return true;
    }
    ...
    bool Spline2dConstraint::AddPointAngleConstraint(const double t,
        ...
        if (normalized_angle >= (M_PI / 2) && normalized_angle < (M_PI * 3 / 2)) {
            y_sign = -1;
        }
