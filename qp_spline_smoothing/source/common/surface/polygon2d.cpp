...
//        CHECK(is_convex_ && other_polygon.is_convex());
    ...
    bool Polygon2d::HasOverlap(const LineSegment2d &line_segment, bool judgePointInFlag) const {
//        CHECK_GE(points_.size(), 3);
        ...
        return GetOverlap2(line_segment, judgePointInFlag);
    }

    bool Polygon2d::GetOverlap2(const LineSegment2d &line_segment, bool judgePointInFlag) const
    {
        if (line_segment.length() <= kMathEpsilon) {
            if (!IsPointIn(line_segment.start())) {
                return false;
